    ...
    private TableActionEvent loadTableEvents(){
        return new TableActionEvent() {
            ...
            public void onView(int row) {
                OrdersD.setTitle(" Assigned Orders");
...
private void updateTable() {
    try {
        ...
        for (DeliveryStaff deliveryStaff : deliveryList) {
            ...
            String vehicle = "-----";
            if (deliveryStaff.getVehichleId() != -1 && VehicleCRUD.readVehicle(deliveryStaff.getVehichleId()) != null) {
                vehicle = VehicleCRUD.readVehicle(deliveryStaff.getVehichleId()).toString();
            }

            String route = "-----";
            if (deliveryStaff.getRoatId() != -1 && RouteCRUD.getRouteById(deliveryStaff.getRoatId()) != null) {
                route = RouteCRUD.getRouteById(deliveryStaff.getRoatId()).toString();
            }

            userModel.addRow(new Object[]{
                id, firstName + " " + lastName, dateOfBirth, gender, address,
                celPhone, telPhone, email, username, role, route, vehicle
            });
   ...
   private void updateOrdersTable() {
    try {
        ...
        for (Object[] order : DeliveryCRUDS.getAssignedOrders(selectedStaffId)) {
            if (order != null) {
                long trackingNumber = (long) order[0];           // PackageID
                String status = (String) order[1];                // Status
                String expectedDeliveryDate = (String) order[2];  // AssignedDate + 3
